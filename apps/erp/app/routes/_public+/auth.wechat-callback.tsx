...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  console.log("[wechat callback] state check", {
    savedState: savedState?.slice(0, 8),
  ...
  console.log(
    "[wechat callback] userInfo",
    userInfo
      ? { unionid: userInfo.unionid?.slice(0, 8), nickname: userInfo.nickname }
      : "FAILED"
  );
  if (!userInfo) {
    return redirect(
  ...
  console.log(
    "[wechat callback] user",
    user ? { id: user.id, email: user.email } : "FAILED"
  );
  ...
  console.log(
    "[wechat callback] generateLink",
    linkData?.properties?.hashed_token
      ? "ok"
      : `FAILED: ${JSON.stringify(linkError)}`
  );
  ...
  const clearStateCookie =
    await wechatStateStorage.destroySession(stateSession);
