...
exports.default = MaintenanceDispatchItemsRoute;
...
function action(_a) {
    return __awaiter(this, arguments, void 0, function (_b) {
        var _c, client, userId, dispatchId, viewClient, dispatch, formData, validation, upsertItem, _d, _e, _f, _g;
        var _h;
        ...
        return __generator(this, function (_j) {
            switch (_j.label) {
                ...
                case 5:
                    formData = _j.sent();
                    return [4 /*yield*/, (0, form_1.validator)(resources_1.maintenanceDispatchItemValidator).validate(formData)];
                case 6:
                    validation = _j.sent();
                    if (validation.error) {
                        return [2 /*return*/, (0, form_1.validationError)(validation.error)];
                    }
                    return [4 /*yield*/, (0, resources_1.upsertMaintenanceDispatchItem)(client, __assign(__assign({}, validation.data), { maintenanceDispatchId: dispatchId, createdBy: validation.data.id ? undefined : userId, 
                            // @ts-expect-error - stfu typescript
                            updatedBy: validation.data.id ? userId : undefined }))];
                case 7:
                    upsertItem = _j.sent();
                    if (!upsertItem.error) return [3 /*break*/, 9];
                    _d = react_router_1.data;
                    _e = [{}];
                    return [4 /*yield*/, (0, session_server_1.flash)(request, (0, auth_1.error)(upsertItem.error, "Failed to save item"))];
                case 8: return [2 /*return*/, _d.apply(void 0, _e.concat([_j.sent()]))];
                case 9:
                    _f = react_router_1.data;
                    _g = [{}];
                    return [4 /*yield*/, (0, session_server_1.flash)(request, (0, auth_1.success)("Item saved"))];
                case 10: return [2 /*return*/, _f.apply(void 0, _g.concat([_j.sent()]))];
            }
        });
    });
}
function MaintenanceDispatchItemsRoute() {
    var _a;
    ...
    var items = (_a = routeData === null || routeData === void 0 ? void 0 : routeData.items) !== null && _a !== void 0 ? _a : [];
    return (<react_1.VStack spacing={4}>
      ...
      {items.length === 0 ? (<react_1.Card>
          ...
          {items.map(function (item) {
                var _a, _b;
                return (<react_1.Card key={item.id}>
              <react_1.CardContent className="py-4">
                <div className="grid grid-cols-4 gap-4 text-sm">
                  <div>
                    <span className="text-muted-foreground">Item:</span>{" "}
                    {(_b = (_a = item.item) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : item.itemId}
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    {item.unitCost
                        ? "$".concat(Number(item.unitCost).toFixed(2))
                        : "-"}
                  </div>
                  <div>
                    <span className="text-muted-foreground">Total Cost:</span>{" "}
                    {item.totalCost
                        ? "$".concat(Number(item.totalCost).toFixed(2))
                        : "-"}
                  </div>
                </div>
              </react_1.CardContent>
