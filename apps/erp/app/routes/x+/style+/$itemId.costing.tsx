...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (itemCost.error) {
    throw redirect(
      ...
      await flash(
        request,
        error(itemCost.error, "Failed to load style costing")
      )
    );
  }
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (updateItemCost.error) {
    throw redirect(
      ...
      await flash(
        request,
        error(updateItemCost.error, "Failed to update style costing")
      )
    );
  }

  throw redirect(
    path.to.styleCosting(itemId),
    await flash(request, success("Updated style costing"))
  );
}

export default function StyleCostingRoute() {
