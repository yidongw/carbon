...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const createStyleSupplier = await upsertSupplierPart(client, {
    ...d,
    companyId,
    createdBy: userId,
  ...
  if (createStyleSupplier.error) {
    return {
      success: false,
      message: "Failed to create style supplier"
    };
  }

  const newSupplierPartId = createStyleSupplier.data?.id;
  ...
  return { success: true, message: "Style supplier created" };
}

export default function NewStyleSupplierRoute() {
  const { itemId } = useParams();

  if (!itemId) throw new Error("itemId not found");

  const routeData = useRouteData<{
    styleSummary: { unitOfMeasureCode: string | null };
  ...
  const onClose = () => navigate(path.to.stylePurchasing(itemId));

  return (
    <SupplierPartForm
      type="Style"
      initialValues={initialValues}
      unitOfMeasureCode={routeData?.styleSummary?.unitOfMeasureCode ?? ""}
      onClose={onClose}
    />
  );
}
