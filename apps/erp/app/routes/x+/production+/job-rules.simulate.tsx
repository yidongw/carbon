...
function evaluateCondition(condition: Condition, job: any): boolean {
  ...
  if (field === "tags") {
    ...
    const ops: any[] = (job.jobMakeMethod ?? []).flatMap(
      (mm: any) => mm.jobOperation ?? []
    );
    ...
    case "in": {
      const vals = Array.isArray(value)
        ...
        : String(value)
            ...
            .map((v) => v.trim());
      return vals.some((v) =>
        Array.isArray(jobValue) ? jobValue.includes(v) : String(jobValue) === v
      );
    }
    case "contains":
                      ...
                      <Td>
                        {r.matchedRule ? (
                          <HStack spacing={1} className="text-sm">
                            ...
                            <span>{r.matchedRule.targetGroupName ?? "—"}</span>
                          </HStack>
                        ) : (
