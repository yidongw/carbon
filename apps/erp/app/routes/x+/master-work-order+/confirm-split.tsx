...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (!isOverlay) {
    const token = overlayToken(
      overlay.to.confirmMasterWorkOrderSplit({ masterWorkOrderId })
    );
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const result = await confirmSplit(client, {
    masterWorkOrderId,
    companyId,
    createdBy: userId,
  ...
  if (result.error) {
    return data(
      { ok: false as const },
      await flash(request, error(result.error, "Failed to confirm split"))
    );
  }
  ...
  if (isOverlay) {
    return data(
      ...
      await flash(
        request,
        success("Split confirmed — bundle work orders created")
      )
    );
  }
  ...
  return redirect(
    path.to.masterWorkOrder(masterWorkOrderId),
    await flash(request, success("Split confirmed"))
  );
}

export default function ConfirmSplitRoute() {
  return null;
}
