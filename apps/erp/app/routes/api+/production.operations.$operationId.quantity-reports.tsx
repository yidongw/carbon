...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const result = await listProductionQuantityReportsForOperation(client, {
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const parsed = createProductionQuantityReportValidator.safeParse(body);
  if (!parsed.success) {
    return data({ error: parsed.error.flatten() }, { status: 400 });
  }

  const { employeeId, notes, lines } = parsed.data;
