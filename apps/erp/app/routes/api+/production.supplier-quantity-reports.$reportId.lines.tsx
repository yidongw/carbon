...
import {
  isJobLocked,
  listJobOperationSupplierQuantityReportLines,
  replaceJobOperationSupplierQuantityReportLines,
  replaceJobOperationSupplierQuantityReportLinesValidator
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const { data: lines, error } = await listJobOperationSupplierQuantityReportLines(
    client,
    {
      reportId,
      companyId,
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const parsed =
    replaceJobOperationSupplierQuantityReportLinesValidator.safeParse(body);
  if (!parsed.success) {
    return data({ error: parsed.error.flatten() }, { status: 400 });
  }

  const result = await replaceJobOperationSupplierQuantityReportLines(client, {
    reportId,
    companyId,
    userId,
