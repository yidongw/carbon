...
function nearestSnap(x: number, y: number) {
  ...
  return getSnapPositions().reduce((best, pos) => {
    const d = Math.hypot(
      pos.x + BUTTON_SIZE / 2 - cx,
      pos.y + BUTTON_SIZE / 2 - cy
    );
    const bd = Math.hypot(
      best.x + BUTTON_SIZE / 2 - cx,
      best.y + BUTTON_SIZE / 2 - cy
    );
...
function LayoutDiagram({ position, active, isXs }: LayoutDiagramProps) {
  ...
  const panel = cn(base, active ? "bg-primary" : "bg-current opacity-50");
  ...
  const diagrams: Record<ChatPosition, ReactNode> = {
    ...
    fullscreen: <div className={cn(panel, "w-full h-full")} />
  };
...
export function FloatingChat() {
  ...
  useEffect(() => {
    ...
    if (snap.x !== x || snap.y !== y) {
      fmAnimate(motionX, snap.x, {
        type: "spring",
        duration: 0.4,
        bounce: 0.15
      });
      fmAnimate(motionY, snap.y, {
        type: "spring",
        duration: 0.4,
        bounce: 0.15
      });
  ...
  const onBtnPointerMove = useCallback(
    (e: React.PointerEvent<HTMLButtonElement>) => {
      ...
      const rawX = Math.max(
        0,
        Math.min(d.startBX + dx, window.innerWidth - BUTTON_SIZE)
      );
      const rawY = Math.max(
        TOPBAR_HEIGHT,
        Math.min(d.startBY + dy, window.innerHeight - BUTTON_SIZE)
      );
      motionX.set(rawX);
      motionY.set(rawY);
    },
        ...
        {isOpen && (
          <motion.div
            ...
            className={cn(
              ...
              effectivePosition === "left-outside" &&
