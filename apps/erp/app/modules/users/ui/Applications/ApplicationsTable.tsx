...
const ApplicationActionMenu = ({
  ...
  const showApproveLoading =
    pendingAction === "approve" || (isReviewing && reviewAction === "approve");
  const showRejectLoading =
    pendingAction === "reject" || (isReviewing && reviewAction === "reject");
      ...
      >
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          >
            <MenuIcon
              icon={
                showRejectLoading ? <Spinner className="h-4 w-4" /> : <LuX />
              }
            />
            <Trans>Reject</Trans>
          </MenuItem>
...
const ApplicationsTable = memo(({ data, count }: ApplicationsTableProps) => {
  ...
  const pendingFilterActive = useMemo(
    () =>
      params.getAll("filter").some((filter) => {
