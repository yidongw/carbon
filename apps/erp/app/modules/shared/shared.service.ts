...
const approvalHandlers: Record<
  ApprovalDocumentTypeName,
...
export async function requestProductionPayApproval(
  ...
  if (created.error || !created.data?.id) {
    return {
      data: null,
      error: {
        message: created.error?.message ?? "Failed to create approval request"
      }
    };
  }
  ...
  if (approverIds.length > 0) {
    try {
      ...
      console.error(
        "Failed to trigger production pay approval notification",
        e
      );
    }
  }
