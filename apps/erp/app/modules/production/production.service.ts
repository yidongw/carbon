...
export async function updateProductionQuantity(
  ...
  // biome-ignore lint/correctness/noUnusedVariables: createdBy is destructured to omit it from the update
  const { id, updatedBy, companyId, createdBy, ...updateData } =
    productionQuantity;
...
export async function getJobOperationPickups(
  ...
  return client
    ...
    .select(
      "*, employee:user!jobOperationPickup_employeeId_fkey(id, firstName, lastName, avatarUrl)"
    )
    .eq("jobOperationId", jobOperationId)
    .order("createdAt", { ascending: false });
}
...
export async function getJobOperationPickup(
  ...
  return client.from("jobOperationPickup").select("*").eq("id", id).single();
}
