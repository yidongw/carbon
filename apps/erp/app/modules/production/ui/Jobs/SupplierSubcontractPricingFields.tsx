...
export function SupplierSubcontractPricingFields({
  ...
  const [pricing, setPricing] = useState<SubcontractPricingPreview | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    void fetch(
      path.to.api.operationSubcontractPricing(jobOperationId, supplierProcessId)
    )
      .then(async (res) => {
        if (!res.ok) {
  ...
  return (
    <VStack
      spacing={3}
      className="w-full rounded-lg border border-border/70 p-3"
    >
