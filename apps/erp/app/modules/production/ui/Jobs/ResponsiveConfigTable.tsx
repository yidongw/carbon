...
> = {
  ...
  getColumnWidthClass?: (column: TColumn, hasReferences: boolean) => string;
...
>(
  ...
  return columns.filter((col) => rows.some((row) => !isFieldEmpty(row, col)));
}
...
>({
  ...
  isFieldEmpty,
  getColumnWidthClass = defaultGetColumnWidthClass as unknown as (
    column: TColumn,
    hasReferences: boolean
  ) => string,
  getCellClassName,
  ...
  const resolveFieldEmpty =
    isFieldEmpty ??
    (defaultIsFieldEmpty as unknown as (row: TRow, column: TColumn) => boolean);
