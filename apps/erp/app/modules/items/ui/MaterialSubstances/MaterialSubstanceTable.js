...
var MaterialSubstancesTable = (0, react_2.memo)(function (_a) {
    ...
    var customColumns = (0, useCustomColumns_1.useCustomColumns)("materialSubstance");
        ...
        var defaultColumns = [
            {
                ...
                cell: function (_a) {
                    var row = _a.row;
                    return row.original.companyId === null ? (<Enumerable_1.Enumerable value={row.original.name}/>) : (<components_1.Hyperlink to={"".concat(path_1.path.to.materialSubstance(row.original.id), "?").concat(params.toString())}>
    ...
    var renderContextMenu = (0, react_2.useCallback)(function (row) {
        return (<>
            <react_1.MenuItem disabled={!permissions.can("update", "parts") || row.companyId === null} onClick={function () {
                navigate("".concat(path_1.path.to.materialSubstance(row.id), "?").concat(params.toString()));
            }}>
              <react_1.MenuIcon icon={<lu_1.LuPencil />}/>
              <macro_1.Trans>Edit Substance</macro_1.Trans>
            </react_1.MenuItem>
            <react_1.MenuItem disabled={!permissions.can("delete", "parts") || row.companyId === null} destructive onClick={function () {
                navigate("".concat(path_1.path.to.deleteMaterialSubstance(row.id), "?").concat(params.toString()));
            }}>
              <react_1.MenuIcon icon={<lu_1.LuTrash />}/>
              <macro_1.Trans>Delete Substance</macro_1.Trans>
            </react_1.MenuItem>
          </>);
    }, [navigate, params, permissions]);
    return (<components_1.Table data={data} columns={columns} count={count} primaryAction={permissions.can("create", "parts") && (<components_1.New label={t(templateObject_5 || (templateObject_5 = __makeTemplateObject(["Material Substance"], ["Material Substance"])))} to={"".concat(path_1.path.to.newMaterialSubstance, "?").concat(params.toString())}/>)} renderContextMenu={renderContextMenu} title={t(templateObject_6 || (templateObject_6 = __makeTemplateObject(["Material Substances"], ["Material Substances"])))}/>);
});
MaterialSubstancesTable.displayName = "MaterialSubstancesTable";
exports.default = MaterialSubstancesTable;
