...
const ApprovalRuleSection = ({
  ...
  const allowsMultiple =
    approvalDocumentTypesWithAmounts.includes(documentType);
  ...
  const nextTierFloor = (lowerBoundAmount: number): number | null =>
    sortedFloors.find((f) => f > lowerBoundAmount) ?? null;
          ...
          <VStack spacing={3} className="items-stretch">
            {activeRules.map((rule) => (
              <ApprovalRuleCard
                ...
                upperBound={
                  allowsMultiple
                    ? nextTierFloor(rule.lowerBoundAmount ?? 0)
                    : null
                }
              />
            ))}
