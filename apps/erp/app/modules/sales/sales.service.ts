...
export async function getOrCreateOpportunityForRecord(
  ...
  if (created.error || !created.data) {
    return {
      data: null,
      error: created.error
    } as Awaited<ReturnType<typeof getOpportunity>>;
  }
...
export async function upsertSalesOrderShipment(
  ...
  if ("id" in salesOrderShipment) {
    const { id, updatedBy, ...rest } =
      salesOrderShipment as typeof salesOrderShipment & { updatedBy?: string };
    const existing = await client
      .from("salesOrderShipment")
      .select("id")
      .eq("id", id)
...
export async function upsertSalesOrderPayment(
  ...
  if ("id" in salesOrderPayment) {
    const { id, updatedBy, ...rest } =
      salesOrderPayment as typeof salesOrderPayment & { updatedBy?: string };
    const existing = await client
      .from("salesOrderPayment")
      .select("id")
      .eq("id", id)
