...
var LinkIssue = function (props) {
    ...
    var _b = useJiraIssues(), issues = _b.issues, fetcher = _b.fetcher;
    var onSearch = (0, react_1.useDebounce)(function (e) {
        if (!e.target.value || e.target.value.trim().length < 3)
            return;
        fetcher.load(path_1.path.to.api.jiraLinkExistingIssue +
            "?actionId=".concat(props.task.id, "&search=").concat(e.target.value));
    }, 300);
    var isSearching = fetcher.state === "loading";
    return (<form_1.ValidatedForm id={id} method="post" action={path_1.path.to.api.jiraLinkExistingIssue} validator={linkIssueValidator} 
    ...
    fetcher={fetcher} resetAfterSubmit onAfterSubmit={function () { return props.onClose(); }}>
      ...
      <react_1.VStack spacing={4}>
        <div className="w-full flex items-center gap-x-2 relative">
          <react_1.Input name="query" type="search" className="w-full" autoComplete="off" placeholder={t(templateObject_1 || (templateObject_1 = __makeTemplateObject(["Search by Jira issue title..."], ["Search by Jira issue title..."])))} onChange={onSearch} disabled={isSearching}/>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Icons_1.JiraIssueStatusBadge status={{
                  ...
                  <div className="mt-2 text-sm text-muted-foreground flex justify-between items-center">
                    <span>
                      {((_b = issue.fields.assignee) === null || _b === void 0 ? void 0 : _b.displayName)
                    ? "Assigned to ".concat(issue.fields.assignee.displayName)
                    : "Unassigned"}
                    </span>
                  </div>
                </div>
