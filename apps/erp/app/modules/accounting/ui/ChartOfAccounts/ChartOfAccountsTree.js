...
var ChartOfAccountsTree = (0, react_2.memo)(function (_a) {
    var _b;
    var data = _a.data, search = _a.search;
    (0, hooks_1.useRealtime)("journal");
    var settings = (0, hooks_1.useSettings)();
    var accountingEnabled = (_b = settings.accountingEnabled) !== null && _b !== void 0 ? _b : false;
    ...
    return (<react_1.ScrollArea className="h-[calc(100dvh-var(--header-height)-61px)] w-full">
        <div className="sticky top-0 z-10 flex h-11 items-center pr-4 text-sm font-medium text-foreground/80 border-b border-border bg-card">
          ...
          {accountingEnabled && (<span className="w-32 text-right px-4">Balance</span>)}
        </div>
        <TreeView_1.TreeView tree={tree} nodes={nodes} getTreeProps={getTreeProps} getNodeProps={getNodeProps} virtualizer={virtualizer} parentRef={parentRef} parentClassName="h-full" renderNode={function (_a) {
            ...
            return (<div className={(0, react_1.cn)("flex h-8 cursor-pointer items-center overflow-hidden pr-4 text-sm group/row", state.selected
                ...
                {/* Balance */}
                {accountingEnabled && (<span className="w-32 text-right tabular-nums shrink-0 text-muted-foreground">
                    {formatCurrency((_b = account.balance) !== null && _b !== void 0 ? _b : 0)}
                  </span>)}

                {/* Actions menu */}
                <react_1.DropdownMenu>
                  <react_1.DropdownMenuTrigger asChild>
                    <button className="ml-1 shrink-0 rounded-md p-1 opacity-0 transition-opacity hover:bg-accent group-hover/row:opacity-100" onClick={function (e) { return e.stopPropagation(); }}>
                      <lu_1.LuEllipsisVertical className="h-3.5 w-3.5 text-muted-foreground"/>
                    </button>
                  </react_1.DropdownMenuTrigger>
                  <react_1.DropdownMenuContent align="end">
                    {isGroup ? (<>
                        {!account.isSystem && (<react_1.DropdownMenuItem onClick={function () { return navigate(account.id); }}>
                            <lu_1.LuPencil className="mr-2 h-4 w-4"/>
                            Edit Group
                          </react_1.DropdownMenuItem>)}
                        <react_1.DropdownMenuItem onClick={function () {
                        return navigate("new-group?parentId=".concat(account.id));
                    }}>
                          <lu_1.LuFolderPlus className="mr-2 h-4 w-4"/>
                          Add Group
                        </react_1.DropdownMenuItem>
                        <react_1.DropdownMenuItem onClick={function () { return navigate("new?parentId=".concat(account.id)); }}>
                          <lu_1.LuFilePlus className="mr-2 h-4 w-4"/>
                          Add Account
                        </react_1.DropdownMenuItem>
                        {!account.isSystem && (<react_1.DropdownMenuItem className="text-destructive" onClick={function () { return navigate("delete/".concat(account.id)); }}>
                            <lu_1.LuTrash2 className="mr-2 h-4 w-4"/>
                            Delete
                          </react_1.DropdownMenuItem>)}
                      </>) : (<>
                        <react_1.DropdownMenuItem onClick={function () { return navigate(account.id); }}>
                          <lu_1.LuPencil className="mr-2 h-4 w-4"/>
                          Edit
                        </react_1.DropdownMenuItem>
                        <react_1.DropdownMenuItem className="text-destructive" onClick={function () { return navigate("delete/".concat(account.id)); }}>
                          <lu_1.LuTrash2 className="mr-2 h-4 w-4"/>
                          Delete
                        </react_1.DropdownMenuItem>
                      </>)}
...
exports.default = ChartOfAccountsTree;
