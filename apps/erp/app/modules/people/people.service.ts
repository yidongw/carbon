...
async function attachDepartmentToSalaryRecords<T extends SalaryListRow>(
  client: SupabaseClient<Database>,
  companyId: string,
  records: T[]
) {
...
function collectSalaryStatusValues(
  ...
  for (const filter of filters) {
    ...
    for (const value of values) {
      if (
        SALARY_PAYMENT_STATUSES.includes(
          value as (typeof SALARY_PAYMENT_STATUSES)[number]
        )
      ) {
        statuses.add(value);
      }
...
export function resolveProductionPayApprovalScope(
  ...
  const statusFilters =
    filters?.filter((f) => f.column === "approvalStatus") ?? [];
...
export async function getProductionPayApprovalRequestRows(
  ...
  if (filterEmployeeIds) {
    const reportsForEmployees =
      await getProductionQuantityReportIdsForEmployees(
        client,
        companyId,
        filterEmployeeIds
      );
...
export async function recordSalaryPayment(
  client: SupabaseClient<Database>,
  data: z.infer<typeof salaryPaymentValidator> & {
    companyId: string;
    paidBy: string;
  }
) {
