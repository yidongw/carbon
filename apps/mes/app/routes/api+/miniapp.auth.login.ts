...
// 用 Response.json(定长 body),微信 wx.request 才能正常收到响应。
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!code) return Response.json({ message: "缺少 code" }, { status: 400 });
  ...
  if (!userId) {
    ...
    if (!phone) {
      return Response.json({ message: "获取微信手机号失败" }, { status: 400 });
    }
