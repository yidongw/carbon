...
//
// 用 jsonResponse(定长 body + Content-Length),而非 react-router 的 data()——
// 后者是分块(chunked)响应,微信 wx.request 对无 Content-Length 的响应会挂起。
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (phone) {
    ...
    return jsonResponse({ ok: true, channel: "phone" });
  }
  ...
  if (email) {
    ...
    if (!user.data || !user.data.active) {
      return jsonResponse({ message: "该邮箱未注册或未激活" }, { status: 404 });
    }
