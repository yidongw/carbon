...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (!code) return data({ message: "请输入验证码" }, { status: 400 });

  let userId = "";

  if (phone) {
    const valid = await checkSmsVerifyCode(phone, code);
    if (!valid) return data({ message: "验证码错误或已过期" }, { status: 401 });

    const user = await findOrCreatePhoneUser(phone);
    if (!user) return data({ message: "关联员工失败" }, { status: 500 });
    if (!user.active) {
      return data({ message: "账号未激活,请联系管理员" }, { status: 403 });
    }
    userId = user.id;
  } else if (email) {
    ...
    if (!user.data || !user.data.active) {
      return data({ message: "该邮箱未注册或未激活" }, { status: 403 });
    }
    userId = user.data.id;
  } else {
