/**
 * Seeds realistic demo/sample data into an EXISTING company (items, suppliers,
 * customers, BOMs, purchase/sales orders, a manufacturing job, etc.).
 *
 * Extracted verbatim from seed-dev-full.ts so it can be reused at runtime (the
 * "Try the demo" flow seeds a per-user demo company via an Inngest job) as well
 * as by the CLI. The caller is responsible for first creating the company, user
...
export async function seedDemoData(
  ...
  for (const s of suppliersData) {
    ...
    const supplierRow = await client.query<{ id: string }>(
      `INSERT INTO supplier (name, "readableId", "supplierTypeId", "supplierStatus", "companyId", "createdBy")
         VALUES ($1, $2, $3, 'Active'::"supplierStatusType", $4, $5) RETURNING id`,
  ...
  for (const c of customersData) {
    ...
    const customerRow = await client.query<{ id: string }>(
      `INSERT INTO customer (name, "readableId", "customerTypeId", "customerStatusId", "companyId", "createdBy")
  ...
  async function getOrCreateSupplierInteraction(
    suppId: string
  ): Promise<string> {
    const existing = await client.query<{ id: string }>(
  ...
  const _steelRodItemId = itemIds["STEEL-ROD-01"];
  ...
  const employeeRow = await client.query<{ id: string }>(
    `SELECT id FROM employee WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  for (const sub of substances) {
    const existing = await client.query<{ id: string }>(
      `SELECT id FROM "materialSubstance" WHERE name = $1 LIMIT 1`,
      [sub.name]
    );
    if (existing.rows.length > 0) {
  ...
  for (const form of forms) {
    const existing = await client.query<{ id: string }>(
      `SELECT id FROM "materialForm" WHERE name = $1 LIMIT 1`,
      [form.name]
    );
    if (existing.rows.length > 0) {
  ...
  if (steelSubstanceId && barFormId) {
    const existing = await client.query<{ id: string }>(
      `SELECT id FROM "materialType" WHERE name = $1 LIMIT 1`,
      ["Carbon Steel Bar"]
    );
    if (existing.rows.length === 0) {
      await client.query(
  ...
  if (!fixtureItemId) {
    const r = await client.query<{ id: string }>(
      `SELECT id FROM item WHERE "readableId" = 'CLAMP-FIXTURE-01' AND "companyId" = $1 LIMIT 1`,
      [companyId]
    );
  ...
  if (!serviceItemId) {
    const r = await client.query<{ id: string }>(
      `SELECT id FROM item WHERE "readableId" = 'MAINT-SVC-01' AND "companyId" = $1 LIMIT 1`,
      [companyId]
    );
  ...
  const existingSupplierPart = await client.query(
    `SELECT 1 FROM "supplierPart" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingSupplierAccount = await client.query(
    `SELECT 1 FROM "supplierAccount" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingCustomerAccount = await client.query(
    `SELECT 1 FROM "customerAccount" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if (bracketItemId2) {
    ...
    if (makeMethodId && cncProcId) {
      const existingMO = await client.query<{ id: string }>(
        `SELECT id FROM "methodOperation" WHERE "makeMethodId" = $1 LIMIT 1`,
        [makeMethodId]
      );
      ...
      if (methodOpId) {
        const existingMOS = await client.query(
          `SELECT 1 FROM "methodOperationStep" WHERE "operationId" = $1 LIMIT 1`,
          [methodOpId]
        );
      ...
      if (makeMethodId && methodOpId) {
        ...
        if (steelItemId2) {
          const existingMM2 = await client.query(
            `SELECT 1 FROM "methodMaterial" WHERE "makeMethodId" = $1 LIMIT 1`,
            [makeMethodId]
          );
  ...
  if (templateId && cncProcId) {
    const existingTMM = await client.query<{ id: string }>(
      `SELECT id FROM "templateMakeMethod" WHERE "templateId" = $1 LIMIT 1`,
      [templateId]
    );
    ...
    if (tmMakeMethodId) {
      const existingTMO = await client.query(
        `SELECT 1 FROM "templateMethodOperation" WHERE "templateMakeMethodId" = $1 LIMIT 1`,
        [tmMakeMethodId]
      );
  ...
  if (procedureId) {
    const existingPS = await client.query(
      `SELECT 1 FROM "procedureStep" WHERE "procedureId" = $1 LIMIT 1`,
      [procedureId]
    );
  ...
  const existingQD = await client.query(
    `SELECT 1 FROM "qualityDocument" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const adminGroupRow = await client.query<{ id: string }>(
    `SELECT id FROM "group" WHERE name = 'Admin' AND "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if (trainingId) {
    const existingTQ = await client.query(
      `SELECT 1 FROM "trainingQuestion" WHERE "trainingId" = $1 LIMIT 1`,
      [trainingId]
    );
    ...
    const existingTA = await client.query<{ id: string }>(
      `SELECT id FROM "trainingAssignment" WHERE "trainingId" = $1 LIMIT 1`,
      [trainingId]
    );
    ...
    if (trainingAssignmentId) {
      const existingTC = await client.query(
        `SELECT 1 FROM "trainingCompletion" WHERE "trainingAssignmentId" = $1 LIMIT 1`,
  ...
  let _ncWorkflowId: string | null = null;
  ...
  if (ncTypeId) {
    const existingNC = await client.query(
      `SELECT 1 FROM "nonConformance" WHERE "companyId" = $1 LIMIT 1`,
      [companyId]
    );
  ...
  if (cncWCId) {
    ...
    const existingMD = await client.query(
      `SELECT 1 FROM "maintenanceDispatch" WHERE "companyId" = $1 LIMIT 1`,
      [companyId]
    );
    if ((existingMD.rowCount ?? 0) === 0) {
      ...
      if (maintenanceScheduleId) {
        const existingMSI = await client.query(
          `SELECT 1 FROM "maintenanceScheduleItem" WHERE "maintenanceScheduleId" = $1 LIMIT 1`,
          [maintenanceScheduleId]
        );
  ...
  if (gaugeTypeId) {
    const existingGauge = await client.query<{ id: string }>(
      `SELECT id FROM gauge WHERE "companyId" = $1 LIMIT 1`,
      [companyId]
    );
    ...
    if (gaugeRecordId) {
      const existingGCR = await client.query(
        `SELECT 1 FROM "gaugeCalibrationRecord" WHERE "gaugeId" = $1 LIMIT 1`,
        [gaugeRecordId]
      );
  ...
  const existingRR = await client.query(
    `SELECT 1 FROM "riskRegister" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingDoc = await client.query<{ id: string }>(
    `SELECT id FROM document WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if (documentId) {
    const existingNote = await client.query(
      `SELECT 1 FROM note WHERE "documentId" = $1 LIMIT 1`,
      [documentId]
    );
  ...
  const existingOpp = await client.query(
    `SELECT 1 FROM opportunity WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingSugg = await client.query(
    `SELECT 1 FROM suggestion WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingNQR = await client.query(
    `SELECT 1 FROM "noQuoteReason" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingPR = await client.query(
    `SELECT 1 FROM "pricingRule" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingWH2 = await client.query(
    `SELECT 1 FROM webhook WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  console.log(
    "40. Seeding attribute data types, custom fields, user attribute categories..."
  );
  ...
  if (adtExisting.rows.length === 0) {
    ...
    const _adt2 = await client.query<{ id: number }>(
      `INSERT INTO "attributeDataType" (label, "isNumeric") VALUES ('Number', true) RETURNING id`
    );
    await client.query(
      `INSERT INTO "attributeDataType" (label, "isBoolean") VALUES ('Boolean', true)`
    );
    await client.query(
      `INSERT INTO "attributeDataType" (label, "isDate") VALUES ('Date', true)`
    );
    await client.query(
      `INSERT INTO "attributeDataType" (label, "isList") VALUES ('List', true)`
    );
  ...
  const existingCF = await client.query(
    `SELECT 1 FROM "customField" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingUAC = await client.query(
    `SELECT 1 FROM "userAttributeCategory" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingAP = await client.query(
    `SELECT 1 FROM "accountingPeriod" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if ((existingAP.rowCount ?? 0) === 0) {
    ...
    accountingPeriodId = (
      await client.query<{ id: string }>(
        `SELECT id FROM "accountingPeriod" WHERE "companyId" = $1 LIMIT 1`,
        [companyId]
      )
    ).rows[0]!.id;
  }
  ...
  const existingJournal = await client.query(
    `SELECT 1 FROM journal WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingHoliday = await client.query(
    `SELECT 1 FROM holiday WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if (steelItemId4) {
    const existingSP2 = await client.query(
      `SELECT 1 FROM "itemSamplingPlan" WHERE "itemId" = $1 LIMIT 1`,
      [steelItemId4]
    );
    ...
    const existingBP = await client.query(
      `SELECT 1 FROM "batchProperty" WHERE "itemId" = $1 LIMIT 1`,
      [steelItemId4]
    );
  ...
  if (bracketItemId3) {
    const existingISL = await client.query(
      `SELECT 1 FROM "itemShelfLife" WHERE "itemId" = $1 LIMIT 1`,
      [bracketItemId3]
    );
  ...
  const suppLocRow = await client.query<{ id: string }>(
    `SELECT id FROM "supplierLocation" WHERE "supplierId" = $1 LIMIT 1`,
    [acmeSupplierId]
  );
  ...
  if (suppLocId2 && cncAbilityId) {
    const existingPartner = await client.query(
      `SELECT 1 FROM partner WHERE id = $1 LIMIT 1`,
      [suppLocId2]
    );
  ...
  if (suppContactId) {
    const existingContractor = await client.query(
      `SELECT 1 FROM contractor WHERE id = $1 LIMIT 1`,
      [suppContactId]
    );
  ...
  if (salaryRecordId) {
    const existingESP = await client.query(
      `SELECT 1 FROM "employeeSalaryPayment" WHERE "salaryRecordId" = $1 LIMIT 1`,
      [salaryRecordId]
    );
  ...
  if (allEmployeesGroupId) {
    const existingJAR = await client.query(
      `SELECT 1 FROM "jobAssignmentRule" WHERE "companyId" = $1 LIMIT 1`,
      [companyId]
    );
    let _jobRuleId: string | null = null;
    ...
    // Get existing job ID
    const jobRow = await client.query<{ id: string }>(
      `SELECT id FROM job WHERE "companyId" = $1 LIMIT 1`,
      [companyId]
    );
    const existingJobId = jobRow.rows[0]?.id;
    if (existingJobId) {
      const existingJGA = await client.query(
        `SELECT 1 FROM "jobGroupAssignment" WHERE "jobId" = $1 LIMIT 1`,
        [existingJobId]
      );
  ...
  if (jobOpId) {
    ...
    const existingJOP = await client.query(
      `SELECT 1 FROM "jobOperationParameter" WHERE "operationId" = $1 LIMIT 1`,
      [jobOpId]
    );
    ...
    const toolItemRow = await client.query<{ id: string }>(
      `SELECT id FROM item WHERE type = 'Tool'::"itemType" AND "companyId" = $1 LIMIT 1`,
      [companyId]
    );
    const toolItemIdForOp = toolItemRow.rows[0]?.id;
    if (toolItemIdForOp) {
      const existingJOT = await client.query(
        `SELECT 1 FROM "jobOperationTool" WHERE "operationId" = $1 LIMIT 1`,
        [jobOpId]
      );
  ...
  const existingSRFQ = await client.query(
    `SELECT 1 FROM "salesRfq" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingReceipt = await client.query(
    `SELECT 1 FROM receipt WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if ((existingReceipt.rowCount ?? 0) === 0 && acmeSupplierId) {
    ...
    const firstPO = await client.query<{ id: string; purchaseOrderId: string }>(
      `SELECT id, "purchaseOrderId" FROM "purchaseOrder" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const recRow = await client.query<{ id: string }>(
      ...
      [
        ...
        acmeSupplierId,
        locationId,
        firstPO.rows[0]?.id ?? null,
        firstPO.rows[0]?.purchaseOrderId ?? null,
        companyId,
        userId
      ]
    );
  ...
  const existingShipment = await client.query(
    `SELECT 1 FROM shipment WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if ((existingShipment.rowCount ?? 0) === 0 && precisionCustomerId) {
    ...
    const firstSO = await client.query<{ id: string; salesOrderId: string }>(
      `SELECT id, "salesOrderId" FROM "salesOrder" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const shipRow = await client.query<{ id: string }>(
      ...
      [
        ...
        precisionCustomerId,
        locationId,
        firstSO.rows[0]?.id ?? null,
        firstSO.rows[0]?.salesOrderId ?? null,
        companyId,
        userId
      ]
    );
  ...
  const existingSI = await client.query(
    `SELECT 1 FROM "salesInvoice" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingPI = await client.query(
    `SELECT 1 FROM "purchaseInvoice" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  if ((existingPI.rowCount ?? 0) === 0 && acmeSupplierId) {
    ...
    const piInteractionId =
      await getOrCreateSupplierInteraction(acmeSupplierId);
  ...
  const existingST = await client.query(
    `SELECT 1 FROM "stockTransfer" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const existingWT = await client.query(
    `SELECT 1 FROM "warehouseTransfer" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  ...
  const soLineRow = await client.query<{ id: string }>(
    `SELECT id FROM "salesOrderLine" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  const soLineId = soLineRow.rows[0]?.id;
  if (soLineId) {
    const existingFul = await client.query(
      `SELECT 1 FROM fulfillment WHERE "salesOrderLineId" = $1 LIMIT 1`,
      [soLineId]
    );
  ...
  if (jobOpId) {
    const existingPE = await client.query(
      `SELECT 1 FROM "productionEvent" WHERE "jobOperationId" = $1 LIMIT 1`,
      [jobOpId]
    );
    ...
    const jobForOpRow = await client.query<{ id: string; jobId: string }>(
      `SELECT "jobId" FROM "jobOperation" WHERE id = $1 LIMIT 1`,
      [jobOpId]
    );
    ...
    if (jobIdForOp) {
      const existingPQR = await client.query(
        `SELECT 1 FROM "productionQuantityReport" WHERE "jobOperationId" = $1 LIMIT 1`,
        [jobOpId]
      );
  ...
  const firstPORow = await client.query<{ id: string }>(
    `SELECT id FROM "purchaseOrder" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  const firstPOId = firstPORow.rows[0]?.id;
  if (firstPOId) {
    const existingPOP = await client.query(
      `SELECT 1 FROM "purchaseOrderPayment" WHERE id = $1 LIMIT 1`,
      [firstPOId]
    );
  ...
  const firstSORow = await client.query<{ id: string }>(
    `SELECT id FROM "salesOrder" WHERE "companyId" = $1 LIMIT 1`,
    [companyId]
  );
  const firstSOId = firstSORow.rows[0]?.id;
  if (firstSOId) {
    const existingSOP = await client.query(
      `SELECT 1 FROM "salesOrderPayment" WHERE id = $1 LIMIT 1`,
      [firstSOId]
    );
    ...
    const existingSOShip = await client.query(
      `SELECT 1 FROM "salesOrderShipment" WHERE id = $1 LIMIT 1`,
      [firstSOId]
    );
  ...
  console.log("61. Seeding material reference tables...");
  {
    ...
    const msubExists = await client.query(
      `SELECT id FROM "materialSubstance" WHERE name='Steel (Dev)' AND "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    let msubId: string;
    ...
    const mformExists = await client.query(
      `SELECT id FROM "materialForm" WHERE name='Sheet (Dev)' AND "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    let mformId: string;
  ...
  console.log("62. Seeding dimension values...");
  {
    ...
    const dims = await client.query<{ id: string; name: string }>(
      `SELECT id, name FROM dimension WHERE "companyGroupId"=$1`,
      [companyGroupId]
    );
    ...
    if (firstDimId && firstDimValueId) {
      const jlRow = await client.query<{ id: string }>(
        `SELECT id FROM "journalLine" WHERE "companyId"=$1 LIMIT 1`,
        [companyId]
      );
      if (jlRow.rows[0]) {
        await client.query(
  ...
  console.log("64. Seeding customer contact and location...");
  {
    const custRow = await client.query<{ id: string }>(
      `SELECT id FROM customer WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const contactRow = await client.query<{ id: string }>(
      `SELECT id FROM contact WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    ...
    if (custId) {
      ...
      if (addrId) {
        const existsCL = await client.query(
          `SELECT id FROM "customerLocation" WHERE "customerId"=$1 LIMIT 1`,
          [custId]
        );
  ...
  console.log("68. Seeding employee shift and type permissions...");
  {
    ...
    const shiftRow = await client.query<{ id: string }>(
      `SELECT id FROM shift WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const etRow = await client.query<{ id: string }>(
      `SELECT id FROM "employeeType" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
  ...
  {
    const existsPlan = await client.query(
      `SELECT id FROM plan WHERE name='Development' LIMIT 1`
    );
  ...
  {
    const uacRow = await client.query<{ id: string }>(
      `SELECT id FROM "userAttributeCategory" LIMIT 1`
    );
    const adtRow = await client.query<{ id: number }>(
      `SELECT id FROM "attributeDataType" WHERE "isText"=true LIMIT 1`
    );
    const uacId = uacRow.rows[0]?.id;
    const adtId = adtRow.rows[0]?.id;
    if (uacId && adtId) {
      const existsUA = await client.query(
        `SELECT id FROM "userAttribute" WHERE name='Department' AND "userAttributeCategoryId"=$1 LIMIT 1`,
        [uacId]
      );
      let uaId: string;
  ...
  console.log("75. Seeding contractor and contractor ability...");
  {
    ...
    const abilRow = await client.query<{ id: string }>(
      `SELECT id FROM ability WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
  ...
  console.log("77. Seeding purchase order sub-records...");
  {
    ...
    const smRow = await client.query<{ id: string }>(
      `SELECT id FROM "shippingMethod" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const stRow = await client.query<{ id: string }>(
      `SELECT id FROM "shippingTerm" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    ...
    if (poId) {
      ...
      const existsPOD = await client.query(
        `SELECT 1 FROM "purchaseOrderDelivery" WHERE id=$1 LIMIT 1`,
        [poId]
      );
  ...
  {
    const piRow = await client.query<{ id: string }>(
      `SELECT id FROM "purchaseInvoice" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    ...
    const polRow = await client.query<{ id: string; purchaseOrderId: string }>(
      `SELECT id, "purchaseOrderId" FROM "purchaseOrderLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const itemRow = await client.query<{ id: string }>(
      `SELECT id FROM item WHERE "companyId"=$1 LIMIT 1`,
    ...
    if (piId) {
      ...
      const existsPID = await client.query(
        `SELECT 1 FROM "purchaseInvoiceDelivery" WHERE id=$1 LIMIT 1`,
        [piId]
      );
      ...
      if (pilId) {
        await client.query(
          ...
          [piId, pilId, 9.0, 10.0, 1, 1, userId]
        );
      }
    }
  ...
  console.log("81. Seeding quote sub-records...");
  {
    ...
    const qlRow = await client.query<{ id: string; quoteId: string }>(
      `SELECT id, "quoteId" FROM "quoteLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const qmmRow = await client.query<{ id: string }>(
      `SELECT id FROM "quoteMakeMethod" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const qopRow = await client.query<{ id: string }>(
      `SELECT id FROM "quoteOperation" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const itemRow = await client.query<{ id: string }>(
      `SELECT id FROM item WHERE "companyId"=$1 LIMIT 1`,
    ...
    const toolRow = await client.query<{ id: string }>(
      `SELECT id FROM item WHERE "companyId"=$1 AND type='Tool' LIMIT 1`,
      [companyId]
    );
  ...
  console.log("82. Seeding quote payment and shipment...");
  {
    ...
    const custLocRow = await client.query<{ id: string; customerId: string }>(
      `SELECT cl.id, cl."customerId" FROM "customerLocation" cl JOIN customer c ON cl."customerId"=c.id WHERE c."companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const custContactRow = await client.query<{ id: string }>(
      `SELECT cc.id FROM "customerContact" cc JOIN customer c ON cc."customerId"=c.id WHERE c."companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const ptRow = await client.query<{ id: string }>(
      `SELECT id FROM "paymentTerm" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const locRow = await client.query<{ id: string }>(
      `SELECT id FROM location WHERE "companyId"=$1 LIMIT 1`,
    ...
    if (quoteId) {
      const existsQP = await client.query(
        `SELECT 1 FROM "quotePayment" WHERE id=$1 LIMIT 1`,
        [quoteId]
      );
      if ((existsQP.rowCount ?? 0) === 0) {
        await client.query(
          ...
          [
            quoteId,
            custRow.rows[0]?.id,
            custLocRow.rows[0]?.id,
            custContactRow.rows[0]?.id,
            ptRow.rows[0]?.id,
            companyId
          ]
        );
      }
      const existsQS = await client.query(
        `SELECT 1 FROM "quoteShipment" WHERE id=$1 LIMIT 1`,
        [quoteId]
      );
  ...
  console.log("84. Seeding supplier quote line price...");
  {
    ...
    const sqlRow = await client.query<{ id: string; supplierQuoteId: string }>(
      `SELECT id, "supplierQuoteId" FROM "supplierQuoteLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
  ...
  console.log("85. Seeding method operation sub-records...");
  {
    const mopRow = await client.query<{ id: string }>(
      `SELECT id FROM "methodOperation" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
  ...
  console.log("86. Seeding procedure parameter...");
  {
    const procRow = await client.query<{ id: string }>(
      `SELECT id FROM procedure WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    if (procRow.rows[0]) {
      await client.query(
  ...
  console.log("88. Seeding quality document step...");
  {
    const qdRow = await client.query<{ id: string }>(
      `SELECT id FROM "qualityDocument" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    if (qdRow.rows[0]) {
      await client.query(
  ...
  {
    const rlRow = await client.query<{
      id: string;
      receiptId: string;
      itemId: string;
    }>(
      `SELECT id, "receiptId", "itemId" FROM "receiptLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const supRow = await client.query<{ id: string }>(
      `SELECT id FROM supplier WHERE "companyId"=$1 LIMIT 1`,
    ...
    const teRow = await client.query<{ id: string }>(
      `SELECT id FROM "trackedEntity" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
  ...
  {
    const ncRow = await client.query<{ id: string }>(
      `SELECT id FROM "nonConformance" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const ncId = ncRow.rows[0]?.id;
    const procRow = await client.query<{ id: string }>(
      `SELECT id FROM process WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const custRow = await client.query<{ id: string }>(
      `SELECT id FROM customer WHERE "companyId"=$1 LIMIT 1`,
    ...
    const jopRow = await client.query<{ id: string; jobId: string }>(
      `SELECT id, "jobId" FROM "jobOperation" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const polRow = await client.query<{ id: string; purchaseOrderId: string }>(
      `SELECT id, "purchaseOrderId" FROM "purchaseOrderLine" WHERE "companyId"=$1 LIMIT 1`,
    ...
    const rlRow = await client.query<{ id: string; receiptId: string }>(
      `SELECT id, "receiptId" FROM "receiptLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const solRow = await client.query<{ id: string; salesOrderId: string }>(
      `SELECT id, "salesOrderId" FROM "salesOrderLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const shlRow = await client.query<{ id: string; shipmentId: string }>(
      `SELECT id, "shipmentId" FROM "shipmentLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    ...
    if (ncId) {
      ...
      const iiRow = await client.query<{ id: string }>(
        `SELECT id FROM "inboundInspection" WHERE "companyId"=$1 LIMIT 1`,
        [companyId]
      );
      if (iiRow.rows[0]) {
        await client.query(
  ...
  {
    const mdRow = await client.query<{ id: string }>(
      `SELECT id FROM "maintenanceDispatch" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const msRow = await client.query<{ id: string }>(
      `SELECT id FROM "maintenanceSchedule" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const empRow = await client.query<{ id: string }>(
      `SELECT id FROM employee WHERE "companyId"=$1 LIMIT 1`,
    ...
    const wcRow = await client.query<{ id: string }>(
      `SELECT id FROM "workCenter" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const itemRow = await client.query<{ id: string }>(
      `SELECT id FROM item WHERE "companyId"=$1 LIMIT 1`,
    ...
    const uomRow = await client.query<{ code: string }>(
      `SELECT code FROM "unitOfMeasure" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const teRow = await client.query<{ id: string }>(
      `SELECT id FROM "trackedEntity" WHERE "companyId"=$1 LIMIT 1`,
  ...
  console.log("93. Seeding job sub-records...");
  {
    ...
    const jopStepRow = await client.query<{ id: string }>(
      `SELECT id FROM "jobOperationStep" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const empRow = await client.query<{ id: string }>(
      `SELECT id FROM employee WHERE "companyId"=$1 LIMIT 1`,
    ...
    const scRow = await client.query<{ id: string }>(
      `SELECT id FROM "scrapReason" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const polRow = await client.query<{ id: string }>(
      `SELECT id FROM "purchaseOrderLine" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const sproc = await client.query<{ id: string }>(
      `SELECT id FROM "supplierProcess" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    ...
    if (jop1 && jop2 && jop1.jobId === jop2.jobId) {
      ...
      if ((existsOp2.rowCount ?? 0) === 0) {
        const jopJmmRow = await client.query<{ id: string }>(
          `SELECT id FROM "jobMakeMethod" WHERE "jobId"=$1 AND "companyId"=$2 LIMIT 1`,
          [jop1.jobId, companyId]
        );
  ...
  {
    const _arRow = await client.query<{ id: string }>(
      `SELECT id FROM "approvalRule" WHERE "companyId"=$1 LIMIT 1`,
      [companyId]
    );
    const poRow = await client.query<{ id: string }>(
      `SELECT id FROM "purchaseOrder" WHERE "companyId"=$1 LIMIT 1`,
  ...
  console.log("98. Seeding demand forecast and projections...");
  {
    ...
    if (dfInserted > 0)
      console.log(`   Created ${dfInserted} demandForecast rows`);
    if (dpInserted > 0)
      console.log(`   Created ${dpInserted} demandProjection rows`);
  }
  ...
  console.log("99. Seeding production quantity report approval requests...");
  {
    const pqrRows = await client.query<{ id: string }>(
      `SELECT id FROM "productionQuantityReport" WHERE "companyId"=$1`,
      [companyId]
    );
