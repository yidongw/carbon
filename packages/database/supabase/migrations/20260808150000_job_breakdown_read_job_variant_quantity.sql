-- Per-SKU breakdown for bundle-less jobs via jobVariantQuantity.
--
-- A master WO that is not split into bundle jobs still allocates its quantity
-- across variant SKUs through the jobVariantQuantity table (jobId, variantItemId,
-- quantity). Previously such a job showed as a single untagged "--" row in the
-- On Jobs breakdown. Read jobVariantQuantity so the parent-level (untagged) part
-- of the breakdown is split per SKU, distributing the job's remaining quantity
-- proportionally to the allocation. Jobs with no allocation still emit one
-- untagged row. The total is unchanged, so number == sum(jobBreakdown) still holds.
