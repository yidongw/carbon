-- Fix: NULL user ids reaching NOT NULL audit columns (same class as
-- 20260706181125_fix-auto-complete-null-user.sql). Found in a sweep of all
-- deployed functions that write createdBy/updatedBy/postedBy from nullable
-- sources.
--
-- 1. sync_purchase_invoice_line_price_change (forked from
--    20260410031805_purchase-invoice-line-interceptor.sql): inserted
--    p_new->>'updatedBy' (nullable) into purchaseInvoicePriceChange."updatedBy"
--    (NOT NULL). Any update to purchaseInvoiceLine unitPrice/quantity that
--    does not stamp updatedBy would fail with 23502 and roll back. Now falls
--    back to the line's createdBy (NOT NULL).
--    (20260616061244 fixed the exchange-rate trigger for exactly this reason
--    but left the interceptor itself unguarded.)
--
-- 2. backflush_job_materials + complete_job_to_inventory (forked verbatim from
--    20260630092517_job-costing-item-dimension.sql): both write
--    itemLedger."createdBy" / pickMethod."createdBy" (NOT NULL) and journal
--    "postedBy" from p_user_id, and complete_job_to_inventory even declares
--    p_user_id DEFAULT NULL. A NULL caller now falls back to the job's
--    creator instead of violating the constraints. The only change to each
--    function body is the p_user_id guard directly after BEGIN.

DROP FUNCTION IF EXISTS sync_purchase_invoice_line_price_change(TEXT, TEXT, JSONB, JSONB);
