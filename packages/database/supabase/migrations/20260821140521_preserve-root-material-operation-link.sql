-- Preserve the root make method's material -> consuming operation link.
--
-- get_method_tree explodes a make method into the rows that become jobMaterial /
-- quoteMaterial. The recursive (nested) arm already carries each material's
-- consuming operation via child."methodOperationId" AS "operationId", but the
-- anchor arm (the root make method's own materials) hardcoded NULL AS
-- "operationId". That asymmetry dropped the operation link for every top-level
-- BOM material, so downstream (jobMaterial.jobOperationId) always came out NULL
-- for root materials even when the BOM author explicitly assigned the material
-- to an operation.
--
-- Because methodMaterial.methodOperationId is nullable, this only activates the
-- link the author explicitly created; unassigned root materials still resolve to
-- NULL. Consumption now follows the assigned operation (e.g. a garment trim
-- assigned to a sewing operation is consumed on the bundle work order that runs
-- that operation, not the master).
--
-- This recreates get_method_tree verbatim from
-- 20260417000300_storage-unit-recreate-dependents.sql, changing only the anchor
-- arm's "operationId" projection.
