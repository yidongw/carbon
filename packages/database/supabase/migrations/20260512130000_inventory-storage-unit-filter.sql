-- Add `storageUnitIds TEXT[]` to get_inventory_quantities so the Inventory
-- table can filter items by the individual storage units they're stocked in.
--
-- Aggregation: collect distinct storageUnit ids from itemLedger rows for the
-- (item, location). NULL storageUnitId rows are excluded so the array only
-- contains real units. Empty array when the item has no storageUnit-bound
-- ledger rows.
--
-- Body otherwise identical to 20260505100000_inventory-storage-type-filter.sql
