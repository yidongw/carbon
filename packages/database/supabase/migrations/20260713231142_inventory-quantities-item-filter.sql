-- Add an optional `item_id` parameter to get_inventory_quantities so
-- single-item callers (getItemQuantities on the item detail loaders) don't
-- force the function to aggregate the entire location's itemLedger history
-- plus every open PO/SO/job just to read one item's quantities.
--
-- item_id defaults to NULL, which preserves the whole-location behavior for
-- the Inventory table. When set, every CTE and the final item scan are
-- restricted to that item.
--
-- Body otherwise identical to 20260512130000_inventory-storage-unit-filter.sql
-- (the prior latest definition) — only the new parameter + filters differ.
...
WITH
  ...
  item_ledgers AS (
    ...
    WHERE "companyId" = company_id
      ...
      AND (item_id IS NULL OR "itemId" = item_id)
    GROUP BY "itemId"
  ),
