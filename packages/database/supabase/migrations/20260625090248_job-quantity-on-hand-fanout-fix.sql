-- Fix over-counting (join fan-out) in get_job_quantity_on_hand.
--
-- The aggregate CTEs filtered to the job's items by joining the raw job_materials
-- rows. When an item is on more than one BoM line those joins multiplied the
-- aggregates: on-hand is joined twice (the total, then the in/not-storage-unit
-- split) so it inflated ~N^2, while PO / sales-order / production supply / demand
-- are joined once so ~N. e.g. an item on 2 lines reported 4x its real on-hand
-- (3 -> 12). The CTEs now join deduped sets (job_material_items /
-- job_material_item_units), so each item is counted once.
