-- Garment WIP correctness: count open BUNDLE jobs, not the master WO.
--
-- Problem: a master work order is split into per-variant bundle jobs. When a
-- bundle receives (e.g. 100) to inventory, the master WO stays open with its
-- own remaining unchanged, so the old logic (count the master, exclude
-- bundles) reported the full master quantity even though those units are
-- already on hand -- overstating "on jobs".
--
-- Fix: a master WO that HAS bundle jobs is represented by those bundles (whose
-- remaining drops as they receive); drop the master job from the count and the
-- untagged breakdown, and count the open bundle jobs per variant instead. Plain
-- jobs / masters without bundles are unchanged. This makes the number equal the
-- sum of the per-SKU jobBreakdown.
  ...
  job_breakdown AS (
    ...
    FROM (
      ...
      SELECT
        ...
        NULL::INTEGER AS "sizeSort",
        SUM(j."productionQuantity" + j."scrapQuantity" - j."quantityReceivedToInventory" - j."quantityShipped") AS "qty"
      FROM "job" j
