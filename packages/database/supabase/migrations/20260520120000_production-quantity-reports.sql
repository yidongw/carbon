...
-- NOTE: reportId is intentionally left nullable. The ERP multi-line report
-- flow always sets it, but the MES production/scrap/rework inserts are not
-- (yet) report-aware and insert quantities with a null reportId rather than
-- failing. Keeping the column nullable lets MES keep working without the
-- report feature instead of breaking it.
