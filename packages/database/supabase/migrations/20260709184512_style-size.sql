...
-- Standard sizes (companyId IS NULL = system-wide).
INSERT INTO "styleSize" ("id", "sizeCode", "sizeName", "companyId", "createdBy")
SELECT id('ssz'), v."sizeCode", v."sizeName", NULL, 'system'
FROM (VALUES
  ('XS',   'Extra Small'),
  ('S',    'Small'),
  ('M',    'Medium'),
  ...
  ('XXXL', '3X Large'),
  ('OS',   'One Size')
) AS v("sizeCode", "sizeName")
WHERE NOT EXISTS (
  SELECT 1 FROM "styleSize" ss
  WHERE ss."sizeCode" = v."sizeCode" AND ss."companyId" IS NULL
);
