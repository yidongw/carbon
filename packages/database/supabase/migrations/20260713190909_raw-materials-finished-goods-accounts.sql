-- Split the single Inventory default account into Raw Materials + Finished Goods.
--
-- Classification is derived from item."replenishmentSystem":
--   'Buy'                   → accountDefault."rawMaterialsAccount"
--   'Make' / 'Buy and Make' → accountDefault."finishedGoodsAccount"
-- The rule is a stable property of the item so the debit side (receipt / job
-- completion) and the credit side (shipment / issue / invoice) always hit the
-- same account — no drift between the two balances.
--
-- The old "inventoryAccount" column is dropped at the very end so every stale
-- reader (TypeScript and PL/pgSQL) fails loudly instead of silently posting to
-- a legacy account.
--
-- Spec: .ai/specs/2026-07-13-raw-materials-finished-goods-accounts.md

-- ============================================================
-- Step 1: Insert "Finished Goods" (1220) for every companyGroup
-- ============================================================
-- 1210 = Inventory (becomes the Raw Materials default), 1230 = WIP,
-- 1240 = Inventory Reserves; 1220 is the free slot in between.
...
JOIN "account" parent
  ...
  AND parent."name" = 'Inventory & Stock'
WHERE NOT EXISTS (
...
ALTER TABLE "accountDefault"
  ADD COLUMN IF NOT EXISTS "rawMaterialsAccount" TEXT;
ALTER TABLE "accountDefault"
  ADD COLUMN IF NOT EXISTS "finishedGoodsAccount" TEXT;

ALTER TABLE "accountDefault"
  DROP CONSTRAINT IF EXISTS "accountDefault_rawMaterialsAccount_fkey";
ALTER TABLE "accountDefault"
  ADD CONSTRAINT "accountDefault_rawMaterialsAccount_fkey"
  FOREIGN KEY ("rawMaterialsAccount") REFERENCES "account"("id")
  ON UPDATE CASCADE ON DELETE RESTRICT;

ALTER TABLE "accountDefault"
  DROP CONSTRAINT IF EXISTS "accountDefault_finishedGoodsAccount_fkey";
ALTER TABLE "accountDefault"
  ADD CONSTRAINT "accountDefault_finishedGoodsAccount_fkey"
  FOREIGN KEY ("finishedGoodsAccount") REFERENCES "account"("id")
  ON UPDATE CASCADE ON DELETE RESTRICT;

-- ============================================================
-- Step 3: Backfill
-- ============================================================
-- rawMaterialsAccount inherits the old inventoryAccount (that's where all
-- purchased stock lives today). finishedGoodsAccount points at the 1220
-- account inserted above; a group with a customized COA (renamed/deleted
-- 'Inventory & Stock' group, or 1220 already used for something else) never
-- got a 'Finished Goods' 1220, so COALESCE back to the old inventoryAccount —
-- no NULL survives and SET NOT NULL below can never fail (the deploy runner
-- retries a failed file over committed partial state). Admins override in
-- settings. Guarded on the old column still existing so a retry after Step 7
-- doesn't reference a dropped column.
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'accountDefault'
      AND column_name = 'inventoryAccount'
  ) THEN
    UPDATE "accountDefault" ad
    SET "rawMaterialsAccount" = ad."inventoryAccount"
    WHERE ad."rawMaterialsAccount" IS NULL;

    UPDATE "accountDefault" ad
    SET "finishedGoodsAccount" = COALESCE(
      (SELECT a."id" FROM "account" a
        INNER JOIN "company" c ON c."companyGroupId" = a."companyGroupId"
        WHERE c."id" = ad."companyId"
          AND a."number" = '1220'
          AND a."name" = 'Finished Goods'
          AND a."isGroup" = false
        LIMIT 1),
      ad."inventoryAccount"
    )
    WHERE ad."finishedGoodsAccount" IS NULL;
  END IF;
END $$;
...
ALTER TABLE "accountDefault"
  ALTER COLUMN "rawMaterialsAccount" SET NOT NULL;
ALTER TABLE "accountDefault"
  ALTER COLUMN "finishedGoodsAccount" SET NOT NULL;

-- ============================================================
-- Step 5: Redefine backflush_job_materials
-- ============================================================
-- Fork of the latest definition (20260710044431_backflush-consumes-cost-
-- adjustments.sql). Only change: the per-material inventory credit resolves
-- Raw Materials vs Finished Goods from the consumed item's replenishment
-- system (a Make subassembly issued to a parent job credits Finished Goods —
-- the same account its own job completion debited).
...
-- ============================================================
-- Step 6: Redefine complete_job_to_inventory
-- ============================================================
-- Fork of the latest definition (20260706182830_fix-null-user-audit-columns.sql).
-- Only change: the WIP-discharge debit resolves Raw Materials vs Finished Goods
-- from the produced item's replenishment system (normally Make → Finished
-- Goods; a job producing a Buy item debits Raw Materials so completion lands
-- where later credits will come from).
...
BEGIN
  ...
  -- Fetch account defaults
  SELECT "rawMaterialsAccount", "finishedGoodsAccount", "workInProgressAccount", "laborAbsorptionAccount"
  INTO STRICT v_raw_materials_account, v_finished_goods_account, v_wip_account, v_labor_absorption_account
  FROM "accountDefault"
  WHERE "companyId" = p_company_id;
...
-- ============================================================
-- Step 7: Drop the old column (last, so stale readers fail loudly
-- and a retry over partial state still has it for the backfill)
-- ============================================================

ALTER TABLE "accountDefault"
  DROP COLUMN IF EXISTS "inventoryAccount";
