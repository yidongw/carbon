...
BEGIN
  ...
  LOOP
    ...
    IF v_labor_cost > 0 AND v_labor_absorption_account IS NOT NULL THEN
      ...
      IF v_labor_accounting_period_id IS NULL THEN
        -- Try to activate an inactive period covering today
  ...
  -- Material consumption accounting for backflushed materials
...
-- Redefine sync_finish_job_operation to call complete_job_to_inventory directly
-- instead of invoking the edge function
