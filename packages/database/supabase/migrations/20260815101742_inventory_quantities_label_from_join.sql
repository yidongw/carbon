-- get_inventory_quantities previously derived the breakdown `label` from
-- itemVariant.valuesKey (replace('|',' · ')). valuesKey is now an opaque,
-- DB-owned uniqueness fingerprint (attributeValueId concat), so that produced
-- garbage on the item-detail inventory pages (which don't re-derive it).
-- Derive the label from the itemVariantAttribute -> itemAttributeValue name
-- join instead (same pattern as the bundleWorkOrders view), and drop the dead
-- valuesKey field from the breakdown JSON. Everything else is unchanged.
...
WITH
  ...
  style_breakdown AS (
    SELECT
      ...
      JSONB_AGG(
        ...
        ORDER BY grp."label" NULLS LAST, grp."variantItemId"
      ) AS "breakdown"
    FROM (
      SELECT
        ...
        COALESCE(
          NULLIF((
            SELECT string_agg(COALESCE(iav."name", iav."code"), ' · '
            ...
            WHERE iva."itemVariantId" = iv."id" AND iva."companyId" = company_id
          ), ''),
          vi."readableIdWithRevision"
        ) AS "label",
      ...
      GROUP BY iv."parentItemId", iv."variantItemId", iv."id", vi."readableIdWithRevision"
  ...
  job_breakdown AS (
    ...
    FROM (
      ...
      GROUP BY mj."itemId", bj."itemId", iv."id", vi."readableIdWithRevision"
      ...
      GROUP BY j."itemId", jvq."variantItemId", iv."id", vi."readableIdWithRevision"
    ) grp
    WHERE grp."qty" IS NOT NULL AND grp."qty" <> 0
    GROUP BY grp."itemId"
  ),
