...
function ChoiceCardGroup(_a) {
    var label = _a.label, value = _a.value, onChange = _a.onChange, options = _a.options, _b = _a.direction, direction = _b === void 0 ? "column" : _b, className = _a.className;
    var groupId = (0, react_1.useId)();
    return (<div className={(0, cn_1.cn)("space-y-2 w-full", className)}>
      {label && (<span className="text-xs font-medium text-muted-foreground">
          {label}
        </span>)}
      <Radio_1.RadioGroup value={value} onValueChange={function (v) { return onChange(v); }} className={(0, cn_1.cn)("gap-2", direction === "row" ? "grid grid-cols-2" : "flex flex-col")}>
        {options.map(function (opt) {
            var inputId = "".concat(groupId, "-").concat(opt.value);
            return (<label key={opt.value} htmlFor={inputId} className={(0, cn_1.cn)("flex cursor-pointer items-start gap-3 rounded-md border border-border bg-accent-40 p-3 transition-colors", "hover:bg-card", "has-[[data-state=checked]]:border-primary has-[[data-state=checked]]:bg-card", "has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring", opt.disabled && "cursor-not-allowed opacity-50")}>
              ...
              <Radio_1.RadioGroupItem id={inputId} value={opt.value} disabled={opt.disabled} className="mt-1 flex-shrink-0"/>
            </label>);
        })}
      </Radio_1.RadioGroup>
    </div>);
