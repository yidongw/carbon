...
export async function findOrCreateWeChatUser(
  ...
  const existing = await serviceRole
    ...
    .maybeSingle();
  console.log(
    "[wechat findOrCreate] existing lookup",
    existing.error
      ? `ERROR: ${JSON.stringify(existing.error)}`
      : existing.data
        ? "found"
        : "not found"
  );
  ...
  console.log(
    "[wechat findOrCreate] createUser",
    authError ? `ERROR: ${JSON.stringify(authError)}` : authUser.user?.id
  );
