...
- **Style writers must not store variantTable on `job.configuration`:** `$jobId.configure.tsx` and `job+/update.tsx` route Style variantTable through `persistStyleJobConfiguration` (writes jvq + clears Style qty JSON from `job.configuration`). Part flat method params still use `variantTableUpdateFields`.
